...
if (Test-Path $Settings) {
    try {
        ...
        if ($e.CLAUDE_CODE_USE_BEDROCK) {
            ...
            Ok "경로: 사내 게이트웨이 ($h)"
            # v1.2: 활성 settings.json 에는 치환된 실제 토큰이 들어가야 정상.
            #   ${...} 가 그대로 남아 있으면 치환 실패(=인증 실패) 신호.
            if ($e.ANTHROPIC_AUTH_TOKEN -match '\$\{') {
                Bad 'AUTH_TOKEN 이 ${...} 참조 그대로 남음 -> 토큰 치환 실패(인증 실패함)'
                Warn "원인: CORP_ANTHROPIC_TOKEN 미설정 상태로 전환됨. setx 후 새 터미널 -> 'cc-switch corp' 재실행."
            } elseif ([string]::IsNullOrWhiteSpace($e.ANTHROPIC_AUTH_TOKEN)) {
                Warn "AUTH_TOKEN 이 비어 있음 -> 게이트웨이 인증 실패 가능"
            } else {
                Ok "AUTH_TOKEN 실제 값 주입됨 (치환 정상)"
            }

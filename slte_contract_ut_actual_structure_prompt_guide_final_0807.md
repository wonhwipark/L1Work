# SLTE TxMngr Contract UT — 실제 UT 구조 학습·업데이트 통합 가이드

## 0. Claude Code에서 이 문서를 사용하는 방법

이 문서는 Claude Code에 로딩한 뒤, 현재 상황에 맞는 진행 절차와 프롬프트를 선택하기 위한 통합 운영 가이드다.

단순히 문서 전체를 한 번에 실행하도록 요청하기보다, 먼저 현재 설치된 스킬과 작업 환경을 확인하고 이 문서의 케이스 중 현재 상황에 맞는 절차를 선택하도록 요청하는 것이 안전하다.

---

### 0.1 사용 전 스킬 설치 및 업데이트

다음 버전 이상이 설치돼 있어야 한다.

| 순서 | 스킬 | 기준 버전 | 주요 신규 기능 |
|---|---|---:|---|
| 1 | `code-analyzer` | `v0.13.19` | `ut-structure-analyze` |
| 2 | `slte-knowledge-manager` | `v0.4.4` | `learn/approve/query-ut-structure` |
| 3 | `code-fix` | `v0.4.5` | `create-contract-ut`, `contract-ut-validate` |
| 4 | `skillsilent` | `v0.2.36` | 위 신규 action의 Silent 정책 등록 |
...
`skillsilent`는 관련 action의 최종 정책을 포함하므로 마지막에 설치하거나 업데이트하는 것이 적절하다.

---

### 0.2 Claude Code 세션 주의사항

이전 버전 스킬을 사용했던 Claude Code 창에서는 오래된 스킬 정의나 Context가 남아 있을 수 있다.

업데이트 후에는 다음 순서를 권장한다.

```text
1. 관련 스킬 업데이트 완료
2. 기존 Claude Code 세션 종료
3. VS Code 또는 PowerShell 새 창 열기
4. 새 Claude Code 세션 시작
5. 이 MD 파일 로딩
6. 아래 통합 시작 프롬프트 실행
```

동일 세션을 계속 사용해야 한다면, 최소한 현재 설치 버전과 신규 action 인식 여부를 먼저 확인해야 한다.

---

### 0.3 Claude Code 최초 로딩용 통합 시작 프롬프트

아래 프롬프트를 이 MD 파일과 함께 Claude Code에 전달한다.

```text
첨부한 `slte_contract_ut_actual_structure_prompt_guide_integrated_0807.md`를 이번 작업의 운영 가이드로 사용해줘.

현재 목표는 Legacy 시나리오에서 정의한 UT 검증 포인트를 SLTE TxMngr 구조에 Mapping하고, 현재 SLTE NR Tx UT 폴더에서 실제로 사용 중인 Test 선언, Fixture, Jomock, 메시지 주입 및 검증 문법을 학습한 뒤 동일한 형식의 Contract UT를 작성하는 것이다.

먼저 다음을 수행해줘.

1. 현재 설치된 관련 스킬과 버전을 확인한다.
2. 다음 action을 사용할 수 있는지 확인한다.
   - code-analyzer: ut-structure-analyze
   - slte-knowledge-manager: learn-ut-structure
   - slte-knowledge-manager: approve-ut-structure
   - slte-knowledge-manager: query-ut-structure
   - code-fix: create-contract-ut
   - code-fix: contract-ut-validate
3. 이 가이드 MD의 Case 중 현재 상황에 해당하는 절차를 선택한다.
4. 아직 실제 SLTE UT 구조가 학습되지 않았다면 Case 1의 `ut-structure-analyze` 단계부터 진행한다.
5. 분석 결과에서 실제 Test 선언 형식, Fixture, Jomock, input 주입, 검증 문법과 대표 파일을 보여준다.
6. 지식매니저 승인 전에는 UT 코드 생성을 진행하지 않는다.
7. 실제 코드에서 관찰되지 않은 TEST_F, TEST, TEST_P 등의 형식을 임의로 사용하지 않는다.
8. 필요한 입력 폴더, Feature ID, Branch, 대표 UT 파일 및 산출물 위치를 정리한다.
9. 실행 가능한 분석 단계는 질문 없이 산출물 생성까지 진행한다.
10. UT 구조 승인과 실제 UT Patch 적용은 별도 승인 단계로 남긴다.
11. 현재 단계에서 사용자가 수행하거나 확인해야 할 다음 작업을 순서대로 안내한다.

이번 Feature의 기본 정보는 다음과 같다.
...
목표:
- Legacy UT는 작성하지 않는다.
- Legacy 시나리오의 API 경계별 input/output 검증 포인트를 정의한다.
- 검증 포인트를 SLTE TxMngr 책임과 API에 Mapping한다.
- 실제 SLTE UT Source Pattern을 학습한 뒤 동일한 형태의 Contract UT를 작성한다.
- Production 소스는 수정하지 않고 UT 폴더만 수정한다.
...
아직 경로가 정해지지 않은 항목은 placeholder로 정리해서 보여주고, 실제 분석을 시작하기 위해 필요한 경로만 요청해줘.
```
...
다음 조건이 모두 충족되기 전에는 실제 UT 코드를 생성하지 않는다.
...
세 조건 중 하나라도 충족되지 않으면 다음 중 하나를 수행한다.

- 추가 대표 UT 파일 탐색
- TxMngr 인접 Feature UT 재분석
- Jomock 또는 Verification 근거 보완
- Candidate 상태 유지
- `review_questions.json`에 부족한 근거 기록

임의로 `TEST_F`, `TEST`, `TEST_P`, 일반적인 GoogleTest assertion 또는 다른 프로젝트의 Mock 문법을 대신 사용해서는 안 된다.

---

### 0.5 Claude Code가 잘못된 형식을 제안한 경우

Claude Code가 실제 SLTE UT와 다른 `TEST_F(...)` 형태를 제안하면 해당 코드를 반영하지 않는다.

아래처럼 요청한다.

```text
현재 제안한 TEST_F 형식은 실제 SLTE TxMngr UT에서 학습된 형식이 아니야.
이 후보는 반영하지 말고 patch_allowed=false로 처리해줘.

다음 순서로 다시 진행해줘.
1. 승인된 UT structure profile 조회
2. allowed_declaration_tokens 확인
3. 가장 가까운 실제 TxMngr UT 대표 파일 확인
4. 실제 Fixture, Jomock, input injection, verification 문법 확인
5. 실제 형식으로 수정 계획 작성
6. 수정 후보를 contract-ut-validate로 재검증

승인 프로파일이 없다면 UT 생성 단계로 넘어가지 말고 Case 1~5 절차를 먼저 진행해줘.
```
...
| 입력 | 설명 |
|---|---|
| Legacy SDM 경로 | BWP switching 실제 수행 로그 |
| Legacy source root | Legacy 1900 NR Tx/BCH/SCC/HAL 관련 코드 |
| SLTE TxMngr source root | 실제 구현 대상 모듈 |
| SLTE NR Tx UT root | 실제 Test 선언과 Jomock을 학습할 폴더 |
| 대표 UT 후보 | TxMngr, SCC Message, HAL CMD와 유사한 UT |
| Branch/Scenario | Legacy 1900, SLTE 대상 Branch |
| Feature ID | `NR_TX_DCI_BWP_SWITCH_PCELL_UL_RANK` |

경로가 확정되지 않았다면 Claude Code가 먼저 필요한 경로 목록과 탐색 기준을 제시하도록 한다.
...
전체 과정이 모두 자동화되는 것은 아니다. 특히 **Legacy 검증 포인트를 SLTE TxMngr 책임/API에 Mapping하는 단계와 필드 assert의 정답을 결정하는 단계는 사람의 아키텍처·도메인 판단이 필요하다.**

| 단계 | 자동화 수준 | 담당 | 실제 공수 |
|---|---|---|---|
| SDM 파싱 | 자동 | `sdm-parser` | 낮음 |
| Legacy flow/field 추출 | 자동 | `code-analyzer` flow/field probe | 중 |
| VP 정의 | 수작업 (근거 추출은 자동) | 사람 + `code-analyzer` 근거 | 높음 |
| **Legacy → SLTE Mapping** | **수작업 (자동 추론 없음)** | **사람 + Mapping 지식 저장** | **가장 높음** |
| 실제 SLTE UT 구조 습득 | 자동 | `code-analyzer ut-structure-analyze` | 낮음 |
| Contract UT 골격 생성 | 반자동 | `code-fix create-contract-ut` | 중 |
| **UT 필드 assert 작성/정답 검토** | **수작업** | **사람 + Legacy 근거** | **높음** |
| 문법·스코프 검증 | 자동 | `code-fix contract-ut-validate` | 낮음 |

운영 원칙:

- Mapping은 Legacy 함수명과 SLTE 함수명을 이름으로 1:1 연결하는 작업이 아니다.
- `field probe` 등은 Legacy 쪽 근거를 제공하지만, SLTE에서 어느 API/필드가 책임을 가져야 하는지는 사람이 결정한다.
- Mapping 결과는 `MAPPED / PARTIAL / MISSING / NOT_APPLICABLE / NEED_REVIEW`로 기록한다.
- `PARTIAL`/`MISSING`은 실제 구현 Gap으로 관리한다.
- 일정 산정 시 Mapping과 필드 assert 검토 시간을 별도로 확보한다.
...
- 비교할 필드
- **필수 assert 필드 (최소 1개 이상)**
- **각 assert 필드의 기대값과 Legacy 근거(SDM 또는 code field probe)**
- 비교하지 않을 공통 필드
- Negative case 후보
- SDM/코드 근거

중요 규칙:
1. mock/API 호출 여부와 필드 값 검증은 별개로 정의해.
2. 각 VP는 호출 검증에 더해 최소 1개 이상의 필드 값 assert를 가져야 해.
3. assert 기대값은 추정하지 말고 Legacy SDM 또는 코드/field probe로 확인된 값만 사용해.
4. 값 근거가 없으면 해당 assert를 확정하지 말고 NEED_REVIEW로 남겨.
5. Legacy 내부 private helper 호출 여부는 검증 포인트로 만들지 마.
```
...
중요: 이 Mapping은 자동 추론 결과를 정답으로 확정하는 단계가 아니야.
code-analyzer/field probe는 Legacy 근거와 SLTE 후보를 제시할 수 있지만, 최종 SLTE API/필드 책임 배치는 사람이 검토·결정해야 해.
이 단계는 전체 과정에서 공수가 가장 큰 구간으로 보고, 불확실한 항목은 NEED_REVIEW로 남겨줘.
...
5. 각 VP는 mock/API 호출 검증과 별도로 **최소 1개 이상의 필드 값 assert**를 포함하도록 생성 Context에 명시해.
6. assert 기대값은 `ut_verification_points`에 기록된 Legacy SDM 또는 code field probe 근거를 사용하고, 근거 없는 값은 추정하지 마.
7. 호출 여부만 검증하고 값 비교가 없는 VP는 `INCOMPLETE_VERIFICATION_POINT`로 표시해.
8. UT 폴더 외 Production 코드는 수정하지 마.
9. 실제 UT 파일 생성 전 허용/금지 token, 대표 예제, VP별 필수 assert 필드와 근거를 먼저 보여줘.
```
...
추가 사람 검토 게이트:
7. 각 VP에 mock/API 호출 검증이 있는지
8. 각 VP에 최소 1개 이상의 실제 필드 값 assert가 있는지
9. assert 기대값이 Legacy SDM 또는 code field probe 근거에서 나온 값인지
10. 호출 여부(.Times 등)만 검증하는 껍데기 VP가 없는지

1~6의 문법/스코프 위반은 기존 validator 정책에 따라 차단해.
7~10은 현재 deterministic validator가 의미 정확성을 완전히 판정할 수 없으므로 사람 검토 게이트로 보고, 미충족 시 validator가 PASS하더라도 UT 승인/반영 단계로 넘어가지 마.
```
...
manual_semantic_gate:
  field_value_assert_per_vp >= 1
  legacy_value_evidence = VERIFIED
  shell_ut_only = false
```
...
6. Legacy 검증 포인트를 SLTE TxMngr에 책임 기준 Mapping
   - 자동 1:1 Mapping으로 확정하지 말고 사람 검토가 필요한 항목은 NEED_REVIEW로 남김
   - 이 단계가 전체 과정에서 가장 공수가 큰 수작업 구간임을 결과에 명시
7. 각 VP에 최소 1개 이상의 필드 값 assert와 Legacy 근거(SDM/field probe)를 지정
8. 승인된 실제 UT 프로파일로 Contract UT 생성
9. TEST_F 등 미관찰 문법을 사용하면 차단
10. 호출 여부만 검증하고 필드 값 assert가 없는 VP는 미완성으로 처리
11. UT 폴더만 수정하고 Production 코드는 수정하지 않음
12. 문법 validator PASS와 별도로 필드 assert의 의미 정확성을 사람이 검토한 뒤 G1/G2 절차로 반영
...
- [ ] 이름 1:1이 아니라 시나리오 책임 기준으로 Mapping했는가
- [ ] SLTE 대응 API/필드 결정은 사람이 검토했는가
- [ ] 불확실 항목을 `NEED_REVIEW`로 남겼는가
- [ ] `MAPPED / PARTIAL / MISSING / NOT_APPLICABLE / NEED_REVIEW` 상태가 기록됐는가
- [ ] Mapping/리뷰 공수를 별도로 고려했는가
...
- [ ] 각 VP에 mock/API 호출 검증이 있는가
- [ ] **각 VP에 최소 1개 이상의 필드 값 assert가 있는가**
- [ ] **assert 기대값이 Legacy SDM 또는 code field probe 근거에서 나왔는가**
- [ ] **호출 여부만 검증하는 껍데기 VP가 없는가**
- [ ] 대상 UT root 밖 파일을 수정하지 않았는가
- [ ] `contract-ut-validate` 결과가 PASS인가
- [ ] validator PASS와 별도로 필드 assert 의미 정확성을 사람이 검토했는가

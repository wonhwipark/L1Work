# INSTALL_SKILL — RCA 설치 가이드

갱신: 2026-06-27 12:21 KST  
대상 스킬: `root-cause-analyzer`  
대상 버전: `convenience_v0.21_kg_root_config`

이 문서는 설치만 설명한다. 설치 후 사용법은 `START_HERE_5.5.md`와 `USER_GUIDE_5.x.md`를 본다.
...
`TargetSkillsDir`에는 `root-cause-analyzer` 폴더 자체가 아니라 그 상위의 `skills` 폴더를 넣는다.

정상 설치 구조:

```text
C:\Users\<사용자>\.claude\skills\root-cause-analyzer\SKILL.md
```

만약 `root-cause-analyzer` 폴더 자체를 지정하면 스크립트가 경고하고 상위 `skills` 폴더를 쓰도록 안내한다.
...
```text
C:\Users\<사용자>\.claude\skills\root-cause-analyzer\SKILL.md 존재
...
`install_skill.ps1`은 Claude Code 스킬만 설치한다. 누적 RCA 지식 저장소(`kg_root`)는 별도로 보존해야 한다.

기본값은 패키지 내부 `rca_kg`다. 반복 사용자는 다음처럼 고정 KG 저장소를 지정한다.

```powershell
.\configure_kg_root.ps1 -KgRoot "D:\AI_Automation\RCA_KG"
```

자세한 내용은 `KG_ROOT_GUIDE.md`를 본다.

...
**작성일:** 2026-08-28  
**Revision:** v2 — Phase 2 부족 Evidence 보강 정책 및 code-analyzer / l1-knowledge-manager 연계 반영  
...
Legacy는 과거 구현의 동작을 알려주는 **Evidence**이고, 현재 SLTE 코드는 구현 가능한 **현실적 Architecture/Interface 제약**을 알려준다. 승인된 Knowledge는 기존 책임/정책/ownership을 보강하는 참고 근거로 사용하되 현재 코드 Fact를 덮어쓰지 않는다.
...
② 현재 SLTE Architecture / Candidate Scope
        +
③ l1-knowledge-manager의 승인된 L1/SLTE 지식
        +
...
| CON-002 | 승인된 ownership/정책 고려 | SLTE+TARGET | Knowledge rule + current code check | PROPOSED |
...
Phase 2 충분성 판정
        ...
        │    ├─ AUTO_CODE_CHECK → code-analyzer
        │    ├─ AUTO_KNOWLEDGE_LOOKUP → l1-knowledge-manager read-only
        │    ├─ USER_INPUT_REQUIRED → 사용자 최소 요청
        │    └─ DECISION_REQUIRED → Phase 3 Open Decision
        │    ↓
        │  Phase 2 재판정
        │
        └─ 충분
...
AUTO_CODE_CHECK:
code-analyzer를 사용해서 현재 SLTE code의 필요한 candidate 주변만 제한적으로 확인한다.
전체 code-analyzer workflow를 다시 실행하지 않는다.

AUTO_KNOWLEDGE_LOOKUP:
l1-knowledge-manager를 read-only로 조회하여 승인된 responsibility / ownership / interface / state-policy / replacement-commonization 지식을 확인한다.
새 Candidate 생성 또는 승인은 하지 않는다.
CANDIDATE_ONLY 지식을 confirmed fact로 사용하지 않는다.

USER_INPUT_REQUIRED:
코드와 승인 지식으로 복구할 수 없는 정보만 사용자에게 최소한으로 요청한다.

DECISION_REQUIRED:
Target 설계 선택이면 Phase 3 Open Decision으로 넘기고, Phase 2 blocker인지 여부를 구분한다.

현재 code-analyzer Fact와 승인 Knowledge가 다르면
CODE_KNOWLEDGE_CONFLICT로 기록하고 자동으로 하나를 정답으로 선택하지 않는다.
...
1. Phase 1 상태
2. Phase 2 상태
3. Phase 2 Missing Evidence Triage
4. code-analyzer 보강 결과
5. Knowledge Manager read-only 조회 결과
6. Code↔Knowledge Conflict / USER_INPUT_REQUIRED / DECISION_REQUIRED
7. Target CL-AIT Purpose/Scope
8. 핵심 Functional Requirement
9. 핵심 NFR/Constraint
10. Legacy KEEP/MODIFY/REMOVE/NEW 요약
11. Open Decision
12. Requirement Gate 결과
13. Target HLD 단계 진행 가능 여부
```
...
# 20. Phase 2 보강에서 두 스킬을 사용하는 기준

```text
질문: "현재 코드가 실제로 어떻게 되어 있는가?"
→ code-analyzer

질문: "기존에 승인된 책임/정책/ownership은 무엇인가?"
→ l1-knowledge-manager read-only

질문: "새 Target은 어떤 방향이어야 하는가?"
→ Target Requirement Decision
```

이 세 질문을 섞지 않는 것이 중요하다.

특히:

```text
Knowledge가 이렇게 되어 있으니 현재 코드도 그럴 것이다.
```
...
```text
현재 코드가 이렇게 되어 있으니 이것이 Target Requirement다.
```

라고 자동 확정하지도 않는다.

Target Requirement는 Legacy Evidence + Current Code Fact + Approved Knowledge + User Direction을 비교한 뒤 결정한다.

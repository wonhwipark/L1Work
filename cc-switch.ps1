<#
  ...
  버전: 1.2  (CC_ENV_SWITCH standalone)

  설계 원칙:
    - settings.json 의 env 가 셸 변수보다 우선 -> 전환 = "프로파일 파일 교체"
    - 반대 경로 변수 잔존 금지 (자격증명 사다리 오염 방지)
    - 토큰은 프로파일 파일(SSOT)에 ${ENV} 참조로만 보관(평문 미저장).
      단, Claude Code 는 settings.json 의 ${ENV} 동적 치환을 지원하지 않으므로
      활성 settings.json 에 쓰는 순간에만 환경변수 실제 값으로 치환한다(v1.2).
...
# 토큰 참조(${CORP_ANTHROPIC_TOKEN}) -> 환경변수 실제 값으로 치환한 내용을 반환.
#   - Claude Code 의 settings.json env 는 ${VAR} 동적 치환을 지원하지 않으므로,
#     활성 settings.json 에 쓸 때만 실제 토큰으로 바꿔 넣는다.
#   - 원본 프로파일 파일(SSOT)은 건드리지 않아 ${...} 참조를 유지(파일에 평문 토큰 미저장).
#   - 반환: @{ Text=[치환된 JSON 문자열]; Ok=[bool]; Reason=[string] }
function Resolve-ProfileToken {
    param([string]$SourceProfile)
    $raw = Get-Content $SourceProfile -Raw
    # ${ENVVAR} 패턴을 사용자/프로세스 환경변수 값으로 치환
    $resolved = [regex]::Replace($raw, '\$\{(?<name>[A-Za-z_][A-Za-z0-9_]*)\}', {
        ...
        if ($null -eq $v) { return $m.Value }   # 못 찾으면 원문 유지 (아래에서 잔존 검사로 차단)
        return $v
    })
    # 치환 후에도 ${...} 가 남아 있으면 = 환경변수 미설정 -> 인증 실패 예고
...
function Set-SettingsAtomic {
    ...
    if ($Kind -eq 'corp') {
        ...
        if (-not $res.Ok) {
            throw "환경변수 미설정으로 토큰 치환 실패: $($res.Reason). setx $($res.Reason) ""<값>"" 후 새 터미널."
        }
        # BOM 없는 UTF-8 로 기록 (Claude Code 파싱 안전)
...
function Invoke-Switch {
    ...
    # corp: 토큰 환경변수 사전 점검 (적용 전에 막아 잘못된 settings.json 생성 방지)
    if ($Kind -eq 'corp') {
        ...
        if ([string]::IsNullOrWhiteSpace($tok)) {
            Write-Err2 "$($Config.CorpTokenVar) 미설정 -> corp 전환 중단 (인증 실패 예방)"
            Write-Warn2 "setx $($Config.CorpTokenVar) ""<발급받은_토큰>"" 실행 후 '새 터미널'에서 재시도."
            return
        }
    }
